...
function Example() {
  ...
  return (
    <div>
      ...
      <button onClick={() => setCount(сount + 1)}> // Установить новое состояние
        Нажми на меня
      </button>
      <button onClick={() => setCount((prevCount)=>prevCount + 1)}> // Модифицировать предыдущее состояние
        Нажми на меня
      </button>
    </div>
  );
...
```
Если мы хотим модифицировать предыдущее состояние а не задать новое, нужно использовать CallBack функцию с использованием предыдущего значение иначе при ассинхронных вызовах мы столкнемся с проблемами.
